...
| `POST /switch/<entity>/turn_on\|turn_off` | Schalter setzen |
...
sind — derzeit nur die Gruppe **Anzeige** mit Ausrichtung, Standby-Seite und
dem Schalter für die PV-Übersicht. Alles andere trägt
...
Drei Dinge sind gegenüber dem reinen Ablesen von Entities hinzugekommen:
...
- **Auswahlfelder** haben kein Dropdown, sondern eine Reihe klickbarer
  ...
  mit der Ausrichtung dreht bzw. Uhr, Zifferblatt und Gesicht zeigt. Beide
...
- **Schalter** bekommen einen Kippschalter. Er schickt bewusst `turn_on` bzw.
  `turn_off` und nicht `toggle`: ein Toggle auf eine veraltete Anzeige hin
  schaltete in die falsche Richtung. Die Anzeige springt sofort um und wird
  vom nächsten Ereignis des Geräts notfalls korrigiert.
- **Farben** werden an ihrer Form erkannt (`text`-Entity, genau sechs Zeichen,
  Wert sieht aus wie Hex) und bekämen einen Farbwähler mit acht Presets.
  Dieser Zweig läuft zurzeit **nie**: die Farbfelder sind in `3f6b9bc` wieder
  entfallen, Symbolfarben stehen fest in `assist-satellit.yaml`. Der Code
  bleibt, weil er an keiner Entity hängt und die Seite damit unverändert
  funktioniert, falls je wieder eine Textfarbe dazukommt.
