...
const ICONS = {
  ...
  // Sonne - steht fuer den einzigen Schalter der Seite (PV Uebersicht).
  sun: ["M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8z", "M12 2v2", "M12 20v2", "M2 12h2",
        "M20 12h2", "M5 5l1.5 1.5", "M17.5 17.5L19 19", "M19 5l-1.5 1.5",
        "M6.5 17.5L5 19"],
};
...
function drawPreview(node, value) {
  ...
  if (value === "Gesicht") {
    // Zwei Augen und ein Mund, in denselben Verhaeltnissen wie page_face:
    // Augen als hohe Rundlinge links und rechts oben, der Mund als breiter
    // Strich darunter.
    node.appendChild(shape("rect", { x: 7.5, y: 8, width: 2, height: 4, rx: 1, fill: "currentColor" }));
    node.appendChild(shape("rect", { x: 14.5, y: 8, width: 2, height: 4, rx: 1, fill: "currentColor" }));
    node.appendChild(shape("rect", { x: 9.5, y: 15, width: 5, height: 1.6, rx: 0.8, fill: "currentColor" }));
    return;
  }
...
// Schalter als Kippschalter. Der Zustand kommt in zwei Formen ueber /events:
// value ist ein echtes Boolean, state die Zeichenkette "ON"/"OFF" - beide
// werden akzeptiert, weil row.sync den erstbesten der beiden bekommt.
//
// ESPHome kennt fuer switch turn_on, turn_off und toggle. Hier wird bewusst
// nicht getoggelt, sondern der Zielzustand geschickt: ein toggle auf eine
// veraltete Anzeige hin schaltet in die falsche Richtung.
function buildSwitchRow(entity, row) {
  const toggle = el("button", "toggle");
  toggle.type = "button";
  const knob = el("i", "toggle-knob");
  toggle.appendChild(knob);

  function an(value) {
    return value === true || value === "ON" || value === "on" || value === 1;
  }

  let state = an(entity.value ?? entity.state);

  toggle.addEventListener("click", () => {
    // Sofort umlegen, damit der Klick sitzt; das Ereignis vom Geraet
    // korrigiert notfalls gleich darauf.
    state = !state;
    row.sync(state);
    post(entity, state ? "turn_on" : "turn_off", "");
  });
  ...
  row.querySelector(".value").appendChild(toggle);
}
...
function buildRow(entity) {
  ...
  if (entity.domain === "select") {
    ...
    icon.appendChild(svg(ICONS.sun));
  } else {
